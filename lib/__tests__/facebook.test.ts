...
Deno.test({
  ...
  async fn(): Promise<any> {
    assertEquals(await facebook.authorize(fakeOakCtx, fakeNext), fakeOakCtx.response.redirect('https://www.facebook.com/v9.0/dialog/oauth?' + facebook.uriFromParams));
    assertEquals(facebook.authURL, 'https://www.facebook.com/v9.0/dialog/oauth?client_id=176079343994638&redirect_uri=http://localhost:3000/test&state={st=state123abc,ds=123456789}&response_type=code&scope=email&');
  }
});
