...
function fakeDeserializer(serializedId: (string | number)) {
  return {
    firstName: 'Hello',
    lastName: 'World'
  };
...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    assert(Object.keys(fakeOakCtx.state)[0] === '_dashport');
    assert(Object.values(fakeOakCtx.state)[0] === {});
  },
});
...
Deno.test({
  ...
  fn(): void {
    ...
    assertThrows(() => testDpOak.authenticate(new TestStrat(), (test: any, testing: any) => 1, fakeDeserializer));
  },
});
...
Deno.test({
  ...
  fn(): void {
    ...
    assertThrows(() => testDpOak.authenticate(new TestStrat(), fakeSerializer, (id: (number | string), testing: (number | string)) => 'foobar'));
  },
});
